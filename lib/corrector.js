 ...
 *
 * Cada etapa reporta su progreso via onProgress(mensaje) para que la
 * interfaz pueda mostrar en tiempo real qué está haciendo Kino.
 */
 ...
 * `label` (ej. "la imagen", "la variación 2 de 3", "la slide 1 de 4") se usa
 * para que los mensajes de progreso identifiquen la pieza en curso.
 */
export async function revisarYCorregir(
  ...
  onProgress(`Kino está validando que el texto de ${label} esté bien…`);
  ...
  for (let i = 0; i < MAX_EDICIONES && mejor.check.verificada && !mejor.check.ok; i++) {
    intentos++;
    onProgress(`Kino está corrigiendo el texto de ${label} (intento ${intentos})…`);
    try {
      ...
      if (!editada) continue;
      onProgress(`Kino está revisando la corrección de ${label}…`);
  ...
  if (!mejor.check.ok && prompt) {
    ...
    for (let i = 0; i < MAX_REGENERACIONES && !mejor.check.ok; i++) {
      intentos++;
      onProgress(`Kino está regenerando ${label} con el texto corregido…`);
      try {
        ...
        onProgress(`Kino está validando el texto de ${label}…`);
...
export async function generarImagenesVerificadas({
  ...
  const attempts = Array.from({ length: count }, async (_, i) => {
    const label = count > 1 ? `la variación ${i + 1} de ${count}` : "la imagen";
    onProgress(`Kino está generando ${label}…`);
    const imgs = await generateImages({ prompt, referenceImages, count: 1, aspectRatio });
    if (imgs.length === 0) return null;
    return revisarYCorregir(imgs[0], {
      expectedText,
      aspectRatio,
      prompt,
      referenceImages,
...
export async function generarSecuenciaVerificada({
  ...
  for (let i = 0; i < prompts.length; i++) {
    const label = `la slide ${i + 1} de ${prompts.length}`;
