...
// ============ Verificador de ortografía en imágenes ============

function buildSpellPrompt(expectedText) {
  return [
    "Lee TODO el texto visible en esta imagen generada para marketing.",
    "1. Detecta el idioma del texto (español o inglés).",
    "2. Verifica en ese idioma: ortografía, tildes, gramática, palabras deformadas, letras repetidas o inventadas (errores típicos de imágenes generadas por IA).",
    expectedText
      ? `3. Además, el texto principal debía decir EXACTAMENTE: "${expectedText}". Verifica que aparece tal cual, sin cambios.`
      : "",
    'Si la imagen no contiene texto, responde ok=true. Responde en el JSON pedido: "ok" true solo si TODO el texto es correcto; en "errores" lista cada problema encontrado (breve, en español).',
  ]
    .filter(Boolean)
    .join("\n");
}

/** Revisa la ortografía del texto de una imagen. Nunca lanza: si el chequeo falla, no bloquea. */
async function checkImageSpelling(imageDataUrl, expectedText) {
  try {
    const data = await callGemini(TEXT_MODEL, {
      contents: [
        {
          role: "user",
          parts: [dataUrlToPart(imageDataUrl), { text: buildSpellPrompt(expectedText) }],
        },
      ],
      generationConfig: {
        responseMimeType: "application/json",
...
/**
 * Genera una imagen y verifica la ortografía de sus textos.
 * Si hay errores, regenera una vez indicando las correcciones.
 */
async function generateOneVerified({ prompt, referenceImages, aspectRatio, expectedText = null }) {
  const images = await generateOne({ prompt, referenceImages, aspectRatio });
  if (images.length === 0) return { image: null, check: null };

  let image = images[0];
  let check = await checkImageSpelling(image, expectedText);

  if (check.verificada && !check.ok) {
    const fixPrompt =
      `${prompt}\n\nATENCIÓN: en un intento anterior el texto de la imagen salió con estos errores: ` +
      `${check.errores.join("; ")}. Corrígelos. Todo texto debe estar perfectamente escrito en su idioma` +
      (expectedText ? ` y el texto principal debe decir EXACTAMENTE: "${expectedText}".` : ".");
    try {
      const retry = await generateOne({ prompt: fixPrompt, referenceImages, aspectRatio });
      if (retry.length > 0) {
        const recheck = await checkImageSpelling(retry[0], expectedText);
        image = retry[0];
        check = { ...recheck, reintentada: true };
      }
    } catch (err) {
      console.warn("Reintento por ortografía falló, se conserva la primera imagen:", err.message);
    }
  }
  return { image, check };
}
...
/**
 * Igual que generateImages pero con verificación ortográfica por imagen.
 * Devuelve { images, verificacion } alineados por índice.
 */
export async function generateImagesVerified({
  prompt,
  referenceImages = [],
  count = 1,
  aspectRatio = "1:1",
  ...
  const attempts = Array.from({ length: count }, () =>
    generateOneVerified({ prompt, referenceImages, aspectRatio, expectedText })
  );
  const settled = await Promise.allSettled(attempts);
...
/**
 * Igual que generateSequence pero con verificación ortográfica por slide.
 * expectedTexts[i] es el texto exacto esperado en la slide i (o null).
 */
export async function generateSequenceVerified({
  prompts,
  referenceImages = [],
  aspectRatio = "1:1",
  expectedTexts = [],
  ...
  for (let i = 0; i < prompts.length; i++) {
    ...
    const { image, check } = await generateOneVerified({
      prompt: finalPrompt,
