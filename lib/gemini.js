...
async function generateOne({ prompt, referenceImages, aspectRatio }) {
  ...
  const instruction = referenceImages.length
    ? `REFERENCIAS OBLIGATORIAS: las ${referenceImages.length} imágenes adjuntas son ejemplos reales de la marca. ` +
      `La nueva imagen DEBE verse como parte de la misma serie/feed que esas referencias: replica su paleta de colores exacta, ` +
      `su estilo (fotografía/ilustración/diseño gráfico), su tratamiento de luz, su tipografía y su estilo de composición. ` +
      `Copia el sistema visual de las referencias; el texto siguiente solo define el CONTENIDO de la pieza, nunca un estilo distinto al de las referencias.\n\n` +
...
export async function buildPrompts({
  ...
  const lineas = [
    ...
    `Imágenes de referencia adjuntas: ${
      parts.length > 0
        ? `sí (${parts.length}) — OBLIGATORIO: extrae su sistema visual (paleta hex, estilo, luz, composición) y escríbelo dentro de cada prompt; las piezas deben verse del mismo feed que las referencias`
        : "no"
    }`,
  ];
